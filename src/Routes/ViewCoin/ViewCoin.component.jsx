...
import TradingViewCart from "../../External/TradingViewChart";
                    ...
                    <div className="flex flex-col w-full  " style={{ flex: "2" }}>
                        ...
                        <div className="flex p-4 w-full justify-between items-center">
                            <div className="flex gap-3">
                                <button onClick={() => setShowDataRange(false)} className={` ${showDataRange ? 'text-black font-bold' : 'bg-blue-500 text-white'} px-3 py-1 rounded-full font-bold`}
                                    
                                >
                                    {coinDetails.symbol} Chart
                                </button>
                                <button onClick={() => setShowDataRange(true)}
                                    className={`px-3 py-1 rounded-full ${showDataRange ? 'bg-blue-500 text-white font-bold' : 'text-black font-bold'} `}>
                                    Trading View
                                </button>
                                ...
                                {!showDataRange && (
                                    <div className="flex justify-end cursor-pointer select-none">
                                        {dateRange.map((date, idx) => (
                                            <span
                                                key={idx}
                                                className={`py-1 px-3 range-button rounded-full ${idx === activeRange ? 'activeRange' : ''}`}
                        ...
                        {showDataRange ?  <TradingViewCart /> :  <Line data={data} options={options} />  }
